...
Seventeen decisions. Nine are settled and struck through; the other eight are still
waiting on you. Every row in the first table is live in the code right now at the value in
**Current** -- to change one, edit the symbol named in **Where** and nothing else.
...
|---|---|---|---|---|---|
| **D1** | Which partition does a key land on? Python inherits librdkafka CRC32; franz-go defaults to murmur2 (what Java uses). | **CRC32 -- matches Python** (hand-written, `internal/kafka/partitioner.go`) | `internal/kafka/policy.go`, `var Partitioner` | Set `kgo.StickyKeyPartitioner(nil)` for Java-compatible murmur2. A *different* partition then goes hot in `hot-partition`/`single_key`. | |
| **D2** | Consumer-group rebalance protocol. Python inherits eager range+roundrobin; franz-go defaults to cooperative-sticky. | eager range+roundrobin | `internal/kafka/policy.go`, `var Balancers` | `rebalance-storm` and `uneven-assignment` behave visibly differently -- they're *about* rebalances. | |
| **D4** | Delete + recreate topics on every run? Python does, unconditionally. | **on -- matches Python** | `internal/kafka/policy.go`, `var RecreateTopicsByDefault` | Set `false`, or pass `--recreate-topics=false`, to leave existing topics and their data alone. | |
| **D9** | Producer buffer full: block or drop? Python lets the error kill the producer permanently. | block (backpressure) | `internal/kafka/policy.go`, `var BlockOnBufferFull` | Affects the numbers during broker-chaos, which is exactly when it happens. | |
| **D10** | Hardcoded `kafka-1/2/3` broker names and `replication_factor <= 3`. Rejects any external cluster. | Python's values kept | `internal/scenario/validate.go`, `var Cluster` | Reassign `Cluster` to relax. Blocks your 24/7 multi-cluster use until you do. | |
| **D13** | Flow instances in flight at once. Python is unbounded and untracked. | **0 = unbounded -- matches Python** | `internal/engine/flow.go`, `var DefaultFlowConcurrency` | Set a positive value to bound the pool; issuance then blocks at saturation and shows as `Saturated` in the snapshot. | |

## Python bugs reproduced on purpose -- all four have since been fixed

You said behaviour must match Python, so each of these started out reproducing a Python
defect, as a single flip with a comment at the site explaining both sides. All four have
since been flipped to the fix; the rows record what changed.

| # | Python's behaviour, originally reproduced | Flip it at | You |
|---|---|---|---|
...
| ~~**D16**~~ | ~~A consumer recreated by `rebalance_consumer` **loses** `failure_rate`/`on_failure`/`max_retries` and reverts to defaults, so a rebalance-storm quietly stops testing failures partway through. The engine agent flagged this as the one worth reconsidering: unlike the others it is **invisible** -- nothing in the output says the replacement lost its config.~~ | **DONE.** `RebalanceConsumer.Commands` in `internal/scenario/incident.go` now carries `victim.Conf` into the `CreateConsumer` command it emits, so the replacement keeps `failure_rate`/`commit_failure_rate`/`on_failure`/`max_retries` -- the executor plumbing (`recreateConsumer` in `internal/engine/engine.go`, wired via `scheduler.go`) already threaded `Conf` through end to end, it was only ever dropped at the point the command was built. `TestRebalanceConsumerCommands` and `TestRebalanceConsumerCarriesFailureConfig` (renamed from `TestRebalanceConsumerDropsFailureConfig`, assertion flipped) pin the fix. | |
| ~~**D17**~~ | ~~An impossible cardinality (`min: 0, max: 5, cardinality: 100`) **hangs forever** with no output.~~ | **DONE.** `generate.BoundFillAttempts` already existed and was exercised by tests, but was never passed from the real run path -- `internal/engine/engine.go`'s `valueFunc` and `internal/engine/flow.go`'s `buildFlow` now both construct their generators with `generate.BoundFillAttempts(cardinalityFillAttempts)` (4096), so an impossible cardinality fails engine construction with a diagnosable error instead of hanging the process. `TestBuildTopicRejectsImpossibleCardinality` and `TestBuildFlowRejectsImpossibleCardinality` (`internal/engine/engine_test.go`) cover the topic and flow-step paths, each with a deadline guard so a regression fails the test suite instead of hanging CI. | |
...
## Raised as blocking -- all but D6 have since been resolved
